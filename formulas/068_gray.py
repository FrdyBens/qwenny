...
def reconstruct(STATE, SIZE, PARAMETERS):
    ...
    return bytes(IGRAY[x] if STATE.get('inv') else GRAY[x] for x in d)[:SIZE]
...
def read(STATE, OFFSET, LENGTH, PARAMETERS):
    ...
    d=base64.b64decode(STATE['data_b64'])
    return bytes((IGRAY if STATE.get('inv') else GRAY)[x] for x in d[OFFSET:OFFSET+LENGTH])
